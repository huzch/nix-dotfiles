...
## 🚀 快速开始 (Quick Start)
> 当前配置会把 `dotfiles/` 通过 Home Manager 直接链接到 `~/.config/`。这样应用配置保持独立，修改仓库里的 dotfiles 后可以立即被对应程序读取，不需要每次都重新构建系统。安装脚本也会把配置仓库和壁纸仓库提前放到新系统用户目录中。
>
> 安装脚本会自动使用两阶段流程：先安装不启用 Home Manager 的基础系统，再把仓库复制到新用户目录，最后在新系统里启用完整配置。你不需要手动编辑 `flake.nix` 或临时注释 Home Manager。
...
脚本内部会先使用 `#<hostName>-install` 配置完成基础系统安装，然后在 `/mnt` 中准备好 `~/Documents/nix-dotfiles`，再执行完整的 `#<hostName>` 配置。这样可以避免 Home Manager 在首装阶段提前引用还不存在的 `~/Documents/nix-dotfiles/dotfiles` 链接目标。
...
等待安装完成并重启你的电脑。脚本会在新系统中准备好 `~/Documents/nix-dotfiles` 和 `~/Pictures/wallpapers`，并已经应用完整系统配置，首次启动后无需再手动 clone 或 rebuild。
...
- **`dotfiles/`**: 核心的美化与应用配置文件（例如 Hyprland, Neovim, Waybar, Rofi 等的配置文件）。这些目录会通过 Home Manager 以 out-of-store symlink 的方式链接到你主目录的 `~/.config/` 下面，方便热更新和单独调试。
...
### 2. 如何修改桌面外观或快捷键？
比如想修改 Hyprland 快捷键、改 Waybar 的样式颜色，或是配置 Neovim：直接在这个仓库的 `dotfiles/` 目录下找到对应文件修改。`~/.config/` 里的相关目录只是指向这里的链接，所以很多应用级改动可以热更新；需要重启、reload 或重新打开应用取决于具体程序。

### 3. 如何应用你的修改？
如果你只修改了 `dotfiles/` 下已有文件，通常不需要执行 `nixos-rebuild`，直接 reload 对应程序即可。

如果你修改了 `.nix` 配置、更新了软件包、调整了服务，或者新增了需要被 Nix 管理的文件，再打开终端执行：
```bash
cd ~/Documents/nix-dotfiles

# 第 1 步：通知 Git 你做了改动（新增或重命名 Nix 管理的文件时尤其重要）
git add .
